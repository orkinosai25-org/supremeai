...
│                                                                             │
│  ┌────────────────────┐  ┌───────────────────────┐  ┌───────────────────┐  │
│  │  B2C SUBSCRIPTIONS │  │  B2B BENCHMARKING API  │  │  SUPREME LLM API  │  │
│  │  £5 – £39 / mo     │  │  £5K – £100K / yr      │  │  Enterprise (Y2+) │  │
│  └────────┬───────────┘  └──────────┬────────────┘  └────────┬──────────┘  │
│           │                         │                         │             │
│  ┌────────▼─────────────────────────▼─────────────────────────▼──────────┐  │
│  │  Layer 5 — Supreme LLM  (proprietary reasoning model)                 │  │
│  ├───────────────────────────────────────────────────────────────────────┤  │
│  │  Layer 4 — Training Dataset Generator  (evaluation pipeline outputs)  │  │
│  ├───────────────────────────────────────────────────────────────────────┤  │
│  │  Layer 3 — Benchmarking Intelligence Platform  (B2B revenue engine)   │  │
│  ├───────────────────────────────────────────────────────────────────────┤  │
│  │  Layer 2 — Multi-Model Orchestration Engine  (parallel · routing)     │  │
│  ├───────────────────────────────────────────────────────────────────────┤  │
│  │  Layer 1 — Azure AI Foundry Model Catalogue  (50+ foundation models)  │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| Tier | Monthly price | Key capability |
|------|:------------:|----------------|
...
3. **Metered overage usage** — pay-as-you-go above tier limits
4. **Benchmark dataset licensing** — evaluation reports sold to enterprises, labs, regulators
5. **Model marketplace commissions** — revenue share from third-party fine-tuned models

### What makes it defensible

The **Benchmark Flywheel**: every user query generates evaluation data → data trains the Supreme LLM → Supreme LLM improves routing → better routing attracts more users → repeat. No competitor can replicate this loop without the same user base.
...
## Slide 6 — Product: Subscription Tiers
...
## Slide 7 — Product: Multi-Model Simultaneous Responses
...
## Slide 8 — Product: The Supreme LLM (Evaluation-Driven Distillation)
...
|-------|----------|------------|---------------------------|
| **Phase 1** | Year 1 | Evaluation + routing intelligence | Routing intelligence model trained on benchmark pipeline outputs; latency/cost/quality scoring engine |
| **Phase 2** | Year 2 | Meta-reasoning orchestration model | Multi-model response synthesis; ensemble reasoning; benchmark scoring engine for B2B |
| **Phase 3** | Year 3 | Domain-specific enterprise reasoning LLM | DevOps automation reasoning; enterprise support agent; code review and audit LLM |
| **Phase 4** | Year 4+ | Autonomous infrastructure agents | Self-improving agentic routing; model procurement agents; AI governance automation |
...
- **Routing intelligence model** — selects the optimal model per request based on task, cost, and latency signals from the benchmark pipeline
- **Benchmark scoring engine** — automated scoring infrastructure that powers the B2B Benchmarking-as-a-Service product
- **Enterprise reasoning assistant layer** — Phase 1 Supreme LLM capable of synthesising multi-model responses for enterprise workflows
- **DevOps orchestration reasoning core** — first domain-specific fine-tune targeting the multi-billion-pound DevOps automation market

This is GPU spend that directly generates revenue — not infrastructure cost.
...
```
                     ...
                     └────────────┬────────────┘
                                  │
          ┌───────────────────────▼──────────────────────┐
          │  6. Better routing attracts more users        │
          └───────────────────────┬──────────────────────┘
                                  │                ▲
          ┌───────────────────────▼──────────────┐ │
          ...
          └───────────────────────┬─────────────┘
                                  │
          ┌───────────────────────▼──────────────┐
          │  3. Best answers become              │
          │     structured dataset               │
          └───────────────────────┬─────────────┘
                                  │
          ┌───────────────────────▼──────────────┐
          │  4. Dataset trains                   │
          │     Supreme LLM reasoning layer      │
          └──────────────────────────────────────┘
```
...
### Stream 4 — Benchmark Dataset Licensing *(high margin, B2B, compounding)*
...
| Stream | Type | Gross Margin | Target Year-2 ARR Share |
|--------|------|:------------:|:-----------------------:|
| B2C Subscriptions | Recurring | ~68 % | 55 % |
| Enterprise Licensing | B2B contract | ~75 % | 20 % |
| Metered Overage | Usage-based | ~65 % | 10 % |
| Benchmark Dataset Licensing | B2B data | ~85 % | 10 % |
| Marketplace Commissions | Platform take-rate | ~90 % | 5 % |

### Unit Economics (blended, target Year 2)

| Metric | Value |
|--------|-------|
| Blended B2C ARPU | £22 / mo |
| Enterprise licensing ACV | £15 K / yr |
...
|---------|:-----------:|:----:|
| Global AI/ML Platform Market | ~£40 B | 38 % |
...
*Key assumptions (investors: reject unexplained numbers, not ambitious ones):*

| Assumption | Value | Rationale |
|------------|-------|-----------|
| Free-to-paid conversion rate | 2 % | Conservative vs. 3–5 % PLG benchmark |
| B2C ARPU (blended) | £20 / mo | Weighted average of Starter/Pro/Unlimited mix |
| B2B benchmarking ACV | £15 K / yr | Based on comparable Scale AI / HELM pricing |
| Churn rate (monthly) | 4 % | Industry average for developer SaaS |
