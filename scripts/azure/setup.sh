...
print_next_steps() {
  ...
  echo "     # Optional: direct OpenAI API key (for future use)"
  echo "     az keyvault secret set --vault-name $KV_NAME --name openai-api-key --value <YOUR_KEY>"
  echo ""
  echo "  2. The App Service application settings are pre-configured with Key Vault references."
  echo "     All model credentials are automatically resolved at runtime – no manual app setting"
  echo "     updates are needed once the secrets above are stored."
  echo ""
  echo "  3. Set environment variables for the SupremeAI API backend (local development only):"
  echo ""
  echo "     export AZURE_KEYVAULT_URI=\$(az keyvault show --name $KV_NAME --query properties.vaultUri -o tsv)"
  echo ""
  echo "  4. Run the SupremeAI API:"
  echo ""
