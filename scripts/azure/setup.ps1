...
function Show-NextSteps {
    ...
    Write-Host "     # Optional: direct OpenAI API key (for future use)"
    Write-Host "     az keyvault secret set --vault-name $kvName --name openai-api-key        --value <KEY>"
    Write-Host ""
    Write-Host "  2. The App Service application settings are pre-configured with Key Vault references."
    Write-Host "     All model credentials are automatically resolved at runtime – no manual app setting"
    Write-Host "     updates are needed once the secrets above are stored."
    Write-Host ""
    Write-Host "  3. Set environment variable for the SupremeAI API backend (local development only):"
    Write-Host ""
    Write-Host '     $env:AZURE_KEYVAULT_URI = (az keyvault show --name ' + $kvName + ' --query properties.vaultUri -o tsv)'
    Write-Host ""
    Write-Host "  4. Run the SupremeAI API:"
    Write-Host ""
